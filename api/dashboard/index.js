    ...
    switch (action) {
        ...
        case "category-trend": {
            try {
                ...
                const historyQuery = {
                    query: "SELECT c.assetId, c.recordedAt, c[\"value\"] FROM c WHERE c.userId = @userId AND c.type = 'AssetHistory' AND (NOT IS_DEFINED(c.isWindowRecord) OR c.isWindowRecord = false) AND (NOT IS_DEFINED(c.isMonthlySnapshot) OR c.isMonthlySnapshot = false) AND ARRAY_CONTAINS(@assetIds, c.assetId)",
                    parameters: [
                        { name: "@userId", value: userId },
                        { name: "@assetIds", value: assetIds }
                    ]
                ...
                const latestByDayAsset = new Map();
                for (const row of historyRows) {
                    const assetId = row.assetId ?? "";
                    const recordedAt = String(row.recordedAt ?? "");
                    if (!assetId || !recordedAt)
                        continue;
                ...
                return (0, responses_1.ok)(result);
            }
            catch (error) {
                context.log(error);
                return (0, responses_1.fail)("SERVER_ERROR", "Failed to build category trend", 500);
            }
        }
        case "stock-trends": {
            try {
                ...
                context.log(error);
                return (0, responses_1.fail)("SERVER_ERROR", "Failed to build stock trends", 500);
            }
        }
        default:
